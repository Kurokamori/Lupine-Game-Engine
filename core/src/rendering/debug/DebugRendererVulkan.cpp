...
// Vulkan-compatible GLSL 450 shaders
namespace VulkanDebugShaders {
...
struct DebugRendererVulkan::Impl {
    GfxDeviceVulkan* device = nullptr;
...
DebugRendererVulkan::DebugRendererVulkan()
...
bool DebugRendererVulkan::initialize(IGfxDevice* device) {
    if (m_impl->initialized) {
        return true;
    }

    m_impl->device = dynamic_cast<GfxDeviceVulkan*>(device);
    if (!m_impl->device) {
    ...
    vertDesc.bytecode = VulkanDebugShaders::DebugLine_Vertex;
    vertDesc.bytecodeSize = strlen(VulkanDebugShaders::DebugLine_Vertex);
    ...
    fragDesc.bytecode = VulkanDebugShaders::DebugLine_Fragment;
    fragDesc.bytecodeSize = strlen(VulkanDebugShaders::DebugLine_Fragment);
    ...
    linePipelineDescDepth.vertexLayout.attributes = {
        {"a_Position", VertexFormat::Float3, offsetof(LineVertex, position), 0, 0},  // binding 0, location 0
        {"a_Color", VertexFormat::Float4, offsetof(LineVertex, color), 0, 1}         // binding 0, location 1
    };
...
void DebugRendererVulkan::shutdown() {
    if (!m_impl->initialized) {
        return;
    }
...
void DebugRendererVulkan::beginFrame() {
...
void DebugRendererVulkan::endFrame() {
...
void DebugRendererVulkan::render(IGfxCommandList* cmd, const Mat4& viewProjection, float lineWidth) {
...
void DebugRendererVulkan::clear() {
    for (int cat = 0; cat < Impl::NUM_CATEGORIES; ++cat) {
        for (int depth = 0; depth < 2; ++depth) {
            m_impl->lineVertices[cat][depth].clear();
            m_impl->lineIndices[cat][depth].clear();
...
void DebugRendererVulkan::draw2DGrid(const DebugGrid& grid) {
    DebugGrid grid2D = grid;
    grid2D.is3D = false;
    renderGrid(nullptr, grid2D, Mat4::Identity());
}

void DebugRendererVulkan::draw3DGrid(const DebugGrid& grid) {
    DebugGrid grid3D = grid;
    grid3D.is3D = true;
    renderGrid(nullptr, grid3D, Mat4::Identity());
}

void DebugRendererVulkan::draw2DBoundingBox(const AABB& bounds, const Color& color, float duration) {
    Vec3 min = bounds.min;
    Vec3 max = bounds.max;
...
void DebugRendererVulkan::draw2DOrientedBoundingBox(const Vec2& center, const Vec2& size, float rotation, const Color& color, float duration) {
...
void DebugRendererVulkan::draw3DBoundingBox(const AABB& bounds, const Color& color, float duration) {
    Vec3 min = bounds.min;
    Vec3 max = bounds.max;
...
void DebugRendererVulkan::draw3DOrientedBoundingBox(const OBB& obb, const Color& color, float duration) {
    Vec3 corners[8];
    for (int i = 0; i < 8; ++i) {
...
void DebugRendererVulkan::draw2DGizmo(const DebugGizmo& gizmo) {
...
void DebugRendererVulkan::draw3DGizmo(const DebugGizmo& gizmo) {
...
void DebugRendererVulkan::drawLineInternal(const Vec3& start, const Vec3& end, const Color& color,
...
void DebugRendererVulkan::drawLine(const Vec3& start, const Vec3& end, const Color& color,
                                    float duration, bool depthTest) {
    drawLineWithCategory(start, end, color, duration, depthTest, LineCategory::Other);
}

void DebugRendererVulkan::drawLineWithCategory(const Vec3& start, const Vec3& end, const Color& color,
...
void DebugRendererVulkan::drawLineStrip(const std::vector<Vec3>& points, const Color& color,
                                        float duration, bool depthTest) {
    drawLineStripWithCategory(points, color, duration, depthTest, LineCategory::Other);
}

void DebugRendererVulkan::drawLineStripWithCategory(const std::vector<Vec3>& points, const Color& color,
                                                     float duration, bool depthTest, LineCategory category) {
    if (points.size() < 2) {
        return;
    }
...
void DebugRendererVulkan::drawRay(const Vec3& origin, const Vec3& direction, const Color& color,
...
void DebugRendererVulkan::drawBox(const Vec3& center, const Vec3& size, const Color& color,
...
void DebugRendererVulkan::drawBoxWithCategory(const Vec3& center, const Vec3& size, const Color& color,
...
void DebugRendererVulkan::drawSphere(const Vec3& center, float radius, const Color& color,
...
void DebugRendererVulkan::drawCircle(const Vec3& center, const Vec3& normal, float radius,
...
void DebugRendererVulkan::drawCircleWithCategory(const Vec3& center, const Vec3& normal, float radius,
...
void DebugRendererVulkan::drawArrow(const Vec3& start, const Vec3& end, const Color& color,
...
void DebugRendererVulkan::drawArrowWithCategory(const Vec3& start, const Vec3& end, const Color& color,
...
void DebugRendererVulkan::drawText(const Vec3& position, const std::string& text,
                                   const Color& color, float size, float duration) {
    debug::EmitDebugText(*this, position, text, color, size, duration);
}

void DebugRendererVulkan::drawAxes(const Vec3& position, float size, float duration) {
...
void DebugRendererVulkan::drawTransform(const Vec3& position, const Mat4& rotation, float size, float duration) {
...
size_t DebugRendererVulkan::getLineCount() const {
    size_t count = 0;
    for (int cat = 0; cat < Impl::NUM_CATEGORIES; ++cat) {
        for (int depth = 0; depth < 2; ++depth) {
...
size_t DebugRendererVulkan::getPrimitiveCount() const {
    size_t count = 0;
    for (int cat = 0; cat < Impl::NUM_CATEGORIES; ++cat) {
        for (int depth = 0; depth < 2; ++depth) {
...
void DebugRendererVulkan::renderGrid(IGfxCommandList*, const DebugGrid& grid, const Mat4&) {
...
void DebugRendererVulkan::drawGizmoArrow(const Vec3& origin, const Vec3& direction, float length, const Color& color, bool depthTest) {
    Vec3 end = origin + direction * length;
    drawArrowWithCategory(origin, end, color, length * 0.1f, 0.0f, depthTest, LineCategory::Gizmo);
}

void DebugRendererVulkan::drawGizmoCircle(const Vec3& center, const Vec3& normal, float radius, const Color& color, bool depthTest) {
    drawCircleWithCategory(center, normal, radius, color, 64, 0.0f, depthTest, LineCategory::Gizmo);
}

void DebugRendererVulkan::drawGizmoBox(const Vec3& center, float size, const Color& color, bool depthTest) {
