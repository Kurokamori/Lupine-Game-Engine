...
/**
 * DirectX 11 implementation of the debug renderer.
 * Provides line, grid, gizmo, and bounding box rendering for the editor.
 */
class DebugRendererDX11 : public DebugRenderer {
public:
    DebugRendererDX11();
    ~DebugRendererDX11() override;
    ...
    // DX11-specific methods for thick line rendering
    void setScreenSize(float width, float height) override;
    void setLineWidth(float width) override;
    void setUseThickLines(bool enable);

private:
    struct Impl;
    std::unique_ptr<Impl> m_impl;

    // Line category for different line widths (note: DX11 doesn't support line width, but we keep for organization)
