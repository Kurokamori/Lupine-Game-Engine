"""
Custom Inspector Widget for Line2D Component
...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                              ...
                              QDoubleSpinBox, QFrame, QScrollArea, QCheckBox,
                              QGroupBox, QSpinBox, QSizePolicy)
from PyQt6.QtCore import Qt, pyqtSignal
from PyQt6.QtGui import QFont, QPainter, QPen, QPainterPath, QColor
import json
import sys
...
class BezierPreviewWidget(QWidget):
...
class Line2DPropertyWidget(QWidget):
    """Custom widget for Line2D component with Bezier curve editing support"""

    value_changed = pyqtSignal(str, object)  # property_name, value
    line_mode_changed = pyqtSignal(bool)  # is_creating_line
    ...
    def _init_ui(self):
        ...
        self.preview_widget = BezierPreviewWidget()
        self.preview_widget.setStyleSheet(f"border: 1px solid {border_color};")
        layout.addWidget(self.preview_widget)

        # Line Creation Button
        self.create_line_button = QPushButton("Create Line (Click in Viewport)")
        self.create_line_button.setStyleSheet(f"""
            QPushButton {{
        ...
        """)
        self.create_line_button.clicked.connect(self._toggle_line_creation)
    ...
    def _toggle_line_creation(self):
        """Toggle line creation mode"""
        self.is_creating_line = not self.is_creating_line

        if self.is_creating_line:
            self.create_line_button.setText("Finish Creating Line")
            self.help_label.setText(
                "Line Creation Mode:\n"
                "• Left-click in viewport to add points\n"
                "• Right-click or Ctrl+click on a point to remove it\n"
                "• Click 'Finish Creating Line' when done"
            )
            self.help_label.setVisible(True)
            # Show Bezier handles in viewport
            self._set_show_handles(True)
        else:
            self.create_line_button.setText("Create Line (Click in Viewport)")
            self.help_label.setVisible(False)
            # Hide Bezier handles in viewport
            self._set_show_handles(False)

        self.line_mode_changed.emit(self.is_creating_line)
    ...
    def _set_show_handles(self, show):
        ...
        if self.editor_bridge and self.component:
            try:
                self.editor_bridge.set_line2d_show_handles(self.component, show)
    ...
    def _set_selected_point(self, index):
        ...
        if self.editor_bridge and self.component:
            try:
                self.editor_bridge.set_line2d_selected_point(self.component, index)
            except Exception as e:
    ...
    def _refresh_points_list(self):
        ...
        # Block signals to prevent triggering _on_point_selected during clear/repopulate
        ...
        try:
            points_json = self.editor_bridge.get_line2d_points(self.component)
            ...
            print(f"Error refreshing Line2D points list: {e}")
        finally:
            self.points_list.blockSignals(False)
    ...
    def _on_point_selected(self, row):
        ...
        if row < 0:
            self.bezier_frame.setVisible(False)
            # Hide handles when no point selected (unless in creation mode)
            if not self.is_creating_line:
                self._set_show_handles(False)
                self._set_selected_point(-1)
                # Notify viewport to stop bezier editing
                self.bezier_edit_changed.emit(None)
            return

        self.bezier_frame.setVisible(True)
        # Show handles when a point is selected
        self._set_show_handles(True)
        self._set_selected_point(row)
        # Notify viewport to enable bezier editing for this component
        ...
        try:
            ...
            if row < len(points):
                ...
                self.symmetric_check.setChecked(pt.get('symmetric', True))
    ...
    def _on_use_bezier_changed(self, state):
        ...
        try:
            self.editor_bridge.set_line2d_point_use_bezier(
                self.component,
    ...
    def _on_control_changed(self):
        ...
        try:
            ...
            # If symmetric, mirror the control points
            if self.symmetric_check.isChecked():
                # When ctrl_in changes, update ctrl_out to be opposite
            ...
            self.editor_bridge.update_line2d_point_bezier(
                self.component,
                self.selected_point_index,
                ctrl_in_x, ctrl_in_y,
    ...
    def add_point(self, x, y):
        ...
        try:
            self.editor_bridge.add_line2d_point(self.component, x, y)
            self._refresh_points_list()
        except Exception as e:
    ...
    def _remove_selected_point(self):
        ...
        if current_row >= 0:
            try:
                self.editor_bridge.remove_line2d_point(self.component, current_row)
                self._refresh_points_list()
            except Exception as e:
    ...
    def _clear_points(self):
        ...
        try:
            self.editor_bridge.clear_line2d_points(self.component)
            self._refresh_points_list()
        except Exception as e:
            print(f"Error clearing points: {e}")

    def is_line_creation_active(self):
        """Check if line creation mode is active"""
        return self.is_creating_line
