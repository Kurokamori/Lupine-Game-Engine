...
Unity-style dialog for adding components to nodes
"""
...
class AddComponentDialog(QDialog):
    """Dialog for adding a component to a node"""

    # Signal emitted when a component type is selected
    component_type_selected = pyqtSignal(str)  # Emits the type name
    ...
    def _setup_ui(self):
        ...
        self.search_input.setPlaceholderText("Type to search component types...")
        ...
        self.tree_widget.setHeaderLabel("Component Types")
    ...
    def _build_nested_tree(self, items_with_categories):
        ...
        Args:
            items_with_categories: List of tuples (category_path, item_data)
                                   where category_path is a string like "Physics/2D"
    ...
    def _load_component_types(self):
        """Load component types from the editor bridge and populate the tree"""
        ...
        for component_type in component_types:
            category_path = component_type.subcategory if component_type.subcategory else "Other"
            if category_path not in items_by_category:
                items_by_category[category_path] = []
            items_by_category[category_path].append(component_type)
        ...
        # Add component types under their respective categories
        for category_path, components in items_by_category.items():
            ...
            if parent_item:
                for component_type in sorted(components, key=lambda t: t.type_name):
                    ...
                    if component_type.is_built_in:
                        display_name += " (Built-in)"
                    type_item.setText(0, display_name)
                    type_item.setData(0, Qt.ItemDataRole.UserRole, component_type.type_name)
                    type_item.setToolTip(0, f"Type: {component_type.type_name}\nPath: {component_type.file_path if component_type.file_path else 'Built-in'}")

                    # Increase font size for component items
    ...
    def _filter_tree_recursive(self, item, search_text):
        ...
        """
        # Check if this is a component (has type data)
    ...
    def get_selected_type(self):
        """Get the selected component type name"""
        return self.selected_type

    def reload_types(self):
        """Reload component types from the editor bridge"""
        self._load_component_types()
